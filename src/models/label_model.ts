...
import { fix_query } from "jxp/libs/query_manipulation";
    ...
    try {
        ...
        if (label.fn) {
            ...
            for (let d of post_data) {
                for (const k of Object.keys(d.label_data)) {
                    keys.add(k);
                }
            }
            for (let key of keys) {
...
LabelSchema.statics.apply_label = async function (data) {
    try {
        ...
        const label = await Label.findById(data.id) as { _id: unknown; name?: string } | null;
...
const apply_labels = async function () {
    try {
        ...
        for (const label of labels) {
            const doc = label as { name?: string };
            console.log(`Applying ${doc.name}`);
            results[doc.name] = await applyLabel(label).catch(err => `Error applying label: ${err.toString()}`);
        }
