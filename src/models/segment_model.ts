...
function ensureConditionIds(segment) {
	...
	for (const cond of segment.conditions) {
		if (!cond) continue;
		if (!cond.id) cond.id = new (ObjectId as unknown as { new (): { toString(): string } })().toString();
	}
}
...
const apply_segments = async function () {
	try {
		...
		for (const segment of segments) {
			...
			if (process.env.NODE_ENV !== "production") console.log(`Applying ${doc.name}`);
			results[doc.name] = await applySegment(segment).catch(err => `Error applying segment: ${err.toString()}`);
		}
